from app import App
from pyIslSdk import SeaViewAppType, Isd4000
import datetime

class Isd4000App(App):
    def __init__(self):
        super().__init__()
    ...
    def subscribe(self):
        ...
        self.device.on_pressure.connect(self.on_pressure)
        ...
        self.device.on_pressure_cal_cert.connect(self.on_pressure_cal_cert)
        self.device.on_temperature_cal_cert.connect(self.on_temperature_cal_cert)
    ...
    def unsubscribe(self):
        ...
        self.device.on_pressure.disconnect(self.on_pressure)
        ...
        self.device.on_pressure_cal_cert.disconnect(self.on_pressure_cal_cert)
        self.device.on_temperature_cal_cert.disconnect(self.on_temperature_cal_cert)
    ...
    def set_logger(self, logger):
        ...
        if self.device is not None:
            self.device.set_logger(logger, SeaViewAppType.isd4000)
    ...
    def on_pressure(self, device, time_us, pressure, depth, pressureRaw):
        time = datetime.datetime.fromtimestamp(time_us/1000000.0)
        print(device, "pressure", time, time_us, pressure, depth)

    def on_temperature(self, device, temperature, temperatureRaw):
    ...
    def on_pressure_cal_cert(self, device, pressure_cal):
        print(device, "pressure cal", pressure_cal)

    def on_temperature_cal_cert(self, device, temperature_cal):
